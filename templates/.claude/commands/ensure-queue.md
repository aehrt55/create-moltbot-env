# Ensure Webhook Queue for Environment

You are ensuring a Cloudflare Queue exists for an environment's webhook cold-start buffering. If the queue already exists, skip creation and just ensure the overlay config is correct.

## Context

When a webhook arrives during container cold start, messages are enqueued to a Cloudflare Queue instead of being dropped. The queue consumer delivers messages after the container starts. This requires:
    ...
    "producers": [
      {
        ...
        "queue": "moltbot-<env-name>-webhook",
      },
    ],
    "consumers": [
      {
        "queue": "moltbot-<env-name>-webhook",
        "dead_letter_queue": "moltbot-<env-name>-webhook-dlq",
        "max_retries": 10,
        "max_batch_size": 5,
        "max_batch_timeout": 5,
      },
...
> - Main: `moltbot-<env-name>-webhook`
> - DLQ: `moltbot-<env-name>-webhook-dlq`
> - Binding: `WEBHOOK_QUEUE` in `overlays/<env-name>/wrangler.jsonc`
>
...
- The `WEBHOOK_QUEUE` binding is optional in the app code — if not configured, webhook handling falls back to fire-and-forget behavior
